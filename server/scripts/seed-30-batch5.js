/**
 * 第五批 30 个商品，严格保证每条使用该商品自带的图片（商品与图片一一对应）
 * 数据源：DummyJSON，每条记录自带 title / price / thumbnail，一一对应
 * 运行: node server/scripts/seed-30-batch5.js
 */
...
// 取 skip=60 区间，排除生鲜，保证每条商品用接口返回的该商品图片
const SKIP = 60;
const LIMIT = 60;
...
async function main() {
  console.log('正在拉取第五批 30 个商品（每条严格使用该商品自带图片）...\n');
  ...
  for (const p of products) {
    ...
    // 仅用本条商品自带的 thumbnail 或 images[0]，保证商品与图片一致
